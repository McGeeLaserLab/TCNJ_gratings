    ...
    def subdivided_pixel(self, rgb_color: list):
        ...
        for i, rgb in enumerate(rgb_color):
            ...
            if total != 0:
                ...
                red_height = int(rgb[0]) / total * self.subpixel_height
                green_height = int(rgb[1]) / total * self.subpixel_height

                j = 0
                # Assign waveform values to subpixel based on RGB widths
                while j < red_height:
                    subpixel[j] = waveform_red[j]
                    j += 1
                while j < red_height + green_height - 1:
                    subpixel[j] = waveform_green[j]
                    j += 1
                while j < self.subpixel_height:
                    subpixel[j] = waveform_blue[j]
                    j += 1
